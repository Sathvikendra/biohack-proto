...
//         toast.success('Congratulations, You have enrolled yourself');
//       }
//     }
//   };
...
//         return (
//           <>
//             {["medical_conditions","surgeries_injuries","medications_supplements","allergies","family_history"
//             // ,"recent_lab_tests"
//           ].map((field) => {
//                 const label = field
//                 .split("_")
...
export default function ProfileSetup() {
  ...
  const [form, setForm] = useState<any>({
    name: "", gender: "", date_of_birth: "", age: "",
    height_cm: "", weight_kg: "", city: "", state: "", country: "",
    occupation: "", work_hours_per_week: "", marital_status: "",
    smoking: false, alcohol: false, caffeine_intake: "",
    exercise_frequency: "", exercise_type: "", avg_steps_per_day: "",
    daily_meals: "", diet_type: "", fruit_veg_servings: "", daily_water_intake_l: "",
    food_allergies: "", stress_level: "", screen_time_hours: "", meditation: false,
    weekday_wakeup: "", weekday_bedtime: "", weekend_wakeup: "", weekend_bedtime: "",
    sleep_duration_hours: "", sleep_quality: "", napping_habits: "", sleep_aids: "",
    medical_conditions: "", surgeries_injuries: "", medications_supplements: "",
    allergies: "", family_history: "", recent_lab_tests: "",
    goal_weight_management: false, goal_energy_performance: false,
    goal_longevity: false, goal_skin_beauty: false,
  });
  ...
  const handleSubmit = async () => {
    ...
    if (error) {
      ...
      if (editMode) {
        ...
        router.push("/dashboard");
        toast.success('Welcome aboard! You are enrolled.');
      }
    }
  };
    ...
    switch(step){
      case 0:
        ...
        <div className="space-y-5">
          {step === 0 && (
            <>
            <p className="text-gray-500 mb-2">Let’s start by getting to know you better.</p>
            <Input name="name" label="Full Name" value={form.name} onChange={handleChange} placeholder="John Doe"/>
            <RadioGroup name="gender" label="Gender" options={["Male","Female","Other"]} value={form.gender} onChange={handleChange}/>
            <Input type="date" name="date_of_birth" label="Date of Birth" value={form.date_of_birth} onChange={handleChange}/>
            <Input name="age" label="Age" value={form.age} readOnly/>
            <div className="grid grid-cols-2 gap-4">
              <Input name="height_cm" label="Height (cm)" value={form.height_cm} onChange={handleChange}/>
              <Input name="weight_kg" label="Weight (kg)" value={form.weight_kg} onChange={handleChange}/>
            </div>
            ...
            <RadioGroup name="occupation" label="Occupation" options={["Desk job","Active job","Mixed"]} value={form.occupation} onChange={handleChange}/>
            <Input name="work_hours_per_week" label="Work hours/week" value={form.work_hours_per_week} onChange={handleChange}/>
            <RadioGroup name="marital_status" label="Marital Status" options={["Single","Married","Divorced"]} value={form.marital_status} onChange={handleChange}/>
            </>
          )}
        </div>
  ...
  return (
    <Layout title={editMode ? "Edit Profile" : "Profile Setup"} showHeader={false}>
      <div className="max-w-3xl mx-auto my-12 p-8 bg-gradient-to-br from-white via-teal-50 to-white rounded-2xl shadow-xl border border-gray-200">
        <h2 className="text-3xl font-bold text-teal-700 mb-3">{editMode ? "Update Your Profile" : "Welcome! Let’s get started"}</h2>
        <p className="text-gray-600 mb-6">Fill in your details below to receive personalized recommendations.</p>
        
        <ProgressBar step={step + 1} total={steps.length} className="mb-6" />
        ...
        <form onSubmit={(e) => { e.preventDefault(); step === steps.length - 1 ? handleSubmit() : next(); }}>
          ...
          <div className="flex justify-between mt-8">
            {step > 0 && (
              <button type="button" onClick={prev} className="px-6 py-2 bg-gray-100 rounded-lg hover:bg-gray-200 transition">
                Back
              </button>
            )}
            <button type="submit" disabled={loading} className="ml-auto px-6 py-2 bg-teal-600 text-white rounded-lg hover:bg-teal-700 transition disabled:opacity-50">
              {step === steps.length - 1 ? (loading ? "Saving…" : editMode ? "Update" : "Finish") : "Next"}
            </button>
          </div>
        </form>
